...
export function normalizePromptRecoveryMode(value) {
  return normalizeRecoveryMode(value, "prompt");
}

export function normalizeReportRecoveryMode(value) {
  return normalizeRecoveryMode(value, "report");
}
...
export function taskStateForLocalPersistence(taskState) {
  ...
  persisted.recovery = { ...(persisted.recovery ?? {}), prompt, report };
...
export function migrateActiveTaskRecovery(taskState) {
  ...
  if (!taskState || typeof taskState !== "object" || Array.isArray(taskState)) throw new TypeError("active task state must be an object");
  const existing = taskState.recovery;
  if (existing == null) return { taskState: { ...taskState, recovery: { prompt: DEFAULT_PROMPT_RECOVERY_MODE, report: DEFAULT_REPORT_RECOVERY_MODE } }, changed: true };
  const prompt = normalizePromptRecoveryMode(existing.prompt ?? DEFAULT_PROMPT_RECOVERY_MODE);
  const report = normalizeReportRecoveryMode(existing.report ?? DEFAULT_REPORT_RECOVERY_MODE);
  return {
    taskState: { ...taskState, recovery: { ...existing, prompt, report } },
    changed: !Object.hasOwn(existing, "prompt") || !Object.hasOwn(existing, "report"),
  };
}
...
/**
 * Report bytes only become recovery-critical after the provider result has
 * been captured. Before that point a restart can continue waiting normally.
 */
