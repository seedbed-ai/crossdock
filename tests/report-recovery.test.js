...
import { DEFAULT_REPORT_RECOVERY_MODE, REPORT_RECOVERY_MODES, assertReportAvailableForRecovery, migrateActiveTaskRecovery, normalizeReportRecoveryMode, taskStateForLocalPersistence } from "../extension/recovery-client.js";

function task(overrides = {}) { return { task_id: "task-1", evidence_policy: { prompt: "omit", report: "full" }, recovery: { prompt: "persist", report: "persist" }, phase: "running", ...overrides }; }
...
test("memory-only captured report fails after restart when full or hash evidence requires bytes", () => {
  for (const evidence of ["full", "hash"]) assert.throws(() => assertReportAvailableForRecovery(task({ recovery: { prompt: "persist", report: "memory" }, evidence_policy: { prompt: "omit", report: evidence }, final_task_url: "https://agent.example/tasks/1", final_report: undefined, phase: "pr-created" })), /memory-only report recovery/);
});

test("omitted durable report evidence never requires recovered report bytes", () => {
  assert.doesNotThrow(() => assertReportAvailableForRecovery(task({ recovery: { prompt: "persist", report: "memory" }, evidence_policy: { prompt: "omit", report: "omit" }, final_task_url: "https://agent.example/tasks/1", phase: "branch-update-clicked" })));
});
